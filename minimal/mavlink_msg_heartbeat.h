                               ...
                               uint8_t type, uint8_t autopilot, uint8_t base_mode, uint32_t custom_mode, uint8_t system_status)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_HEARTBEAT_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_HEARTBEAT_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_heartbeat_t* heartbeat_final = (mavlink_heartbeat_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), heartbeat_final, MAVLINK_MSG_ID_HEARTBEAT_LEN);
...
static inline void mavlink_msg_heartbeat_decode(const mavlink_message_t* msg, mavlink_heartbeat_t* heartbeat)
{
   ...
   // };
    ...
    memcpy(heartbeat, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(heartbeat, decrypt_char, len);
