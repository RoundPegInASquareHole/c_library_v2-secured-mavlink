                               ...
                               uint64_t time_usec, uint8_t target_system, uint8_t cam_idx, uint16_t img_idx, uint8_t event_id, float p1, float p2, float p3, float p4)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_CAMERA_STATUS_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_CAMERA_STATUS_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_camera_status_t* camera_status_final = (mavlink_camera_status_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), camera_status_final, MAVLINK_MSG_ID_CAMERA_STATUS_LEN);
...
static inline void mavlink_msg_camera_status_decode(const mavlink_message_t* msg, mavlink_camera_status_t* camera_status)
{
   ...
   // };
    ...
    memcpy(camera_status, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(camera_status, decrypt_char, len);
