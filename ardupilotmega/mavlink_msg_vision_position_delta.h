                               ...
                               uint64_t time_usec, uint64_t time_delta_usec, const float *angle_delta, const float *position_delta, float confidence)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_VISION_POSITION_DELTA_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_VISION_POSITION_DELTA_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_vision_position_delta_t* vision_position_delta_final = (mavlink_vision_position_delta_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), vision_position_delta_final, MAVLINK_MSG_ID_VISION_POSITION_DELTA_LEN);
...
static inline void mavlink_msg_vision_position_delta_decode(const mavlink_message_t* msg, mavlink_vision_position_delta_t* vision_position_delta)
{
   ...
   // };
    ...
    memcpy(vision_position_delta, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(vision_position_delta, decrypt_char, len);
