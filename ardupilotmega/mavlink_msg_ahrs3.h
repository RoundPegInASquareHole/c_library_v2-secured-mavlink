                               ...
                               float roll, float pitch, float yaw, float altitude, int32_t lat, int32_t lng, float v1, float v2, float v3, float v4)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_AHRS3_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_AHRS3_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_ahrs3_t* ahrs3_final = (mavlink_ahrs3_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), ahrs3_final, MAVLINK_MSG_ID_AHRS3_LEN);
...
static inline void mavlink_msg_ahrs3_decode(const mavlink_message_t* msg, mavlink_ahrs3_t* ahrs3)
{
   ...
   // };
    ...
    memcpy(ahrs3, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
