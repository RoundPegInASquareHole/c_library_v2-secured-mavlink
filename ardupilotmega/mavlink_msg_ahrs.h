                               ...
                               float omegaIx, float omegaIy, float omegaIz, float accel_weight, float renorm_val, float error_rp, float error_yaw)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_AHRS_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_AHRS_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_ahrs_t* ahrs_final = (mavlink_ahrs_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), ahrs_final, MAVLINK_MSG_ID_AHRS_LEN);
...
static inline void mavlink_msg_ahrs_decode(const mavlink_message_t* msg, mavlink_ahrs_t* ahrs)
{
   ...
   // };
    ...
    memcpy(ahrs, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
