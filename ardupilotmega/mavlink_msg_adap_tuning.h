                               ...
                               uint8_t axis, float desired, float achieved, float error, float theta, float omega, float sigma, float theta_dot, float omega_dot, float sigma_dot, float f, float f_dot, float u)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_ADAP_TUNING_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_ADAP_TUNING_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_adap_tuning_t* adap_tuning_final = (mavlink_adap_tuning_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), adap_tuning_final, MAVLINK_MSG_ID_ADAP_TUNING_LEN);
...
static inline void mavlink_msg_adap_tuning_decode(const mavlink_message_t* msg, mavlink_adap_tuning_t* adap_tuning)
{
   ...
   // };
    ...
    memcpy(adap_tuning, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(adap_tuning, decrypt_char, len);
