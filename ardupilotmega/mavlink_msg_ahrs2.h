                               ...
                               float roll, float pitch, float yaw, float altitude, int32_t lat, int32_t lng)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_AHRS2_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_AHRS2_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_ahrs2_t* ahrs2_final = (mavlink_ahrs2_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), ahrs2_final, MAVLINK_MSG_ID_AHRS2_LEN);
...
static inline void mavlink_msg_ahrs2_decode(const mavlink_message_t* msg, mavlink_ahrs2_t* ahrs2)
{
   ...
   // };
    ...
    memcpy(ahrs2, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
