                               ...
                               uint8_t cmd_id, uint8_t status)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_GOPRO_SET_RESPONSE_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_GOPRO_SET_RESPONSE_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_gopro_set_response_t* gopro_set_response_final = (mavlink_gopro_set_response_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), gopro_set_response_final, MAVLINK_MSG_ID_GOPRO_SET_RESPONSE_LEN);
...
static inline void mavlink_msg_gopro_set_response_decode(const mavlink_message_t* msg, mavlink_gopro_set_response_t* gopro_set_response)
{
   ...
   // };
    ...
    memcpy(gopro_set_response, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(gopro_set_response, decrypt_char, len);
