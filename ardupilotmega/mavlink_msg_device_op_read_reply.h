                               ...
                               uint32_t request_id, uint8_t result, uint8_t regstart, uint8_t count, const uint8_t *data, uint8_t bank)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_DEVICE_OP_READ_REPLY_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_DEVICE_OP_READ_REPLY_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_device_op_read_reply_t* device_op_read_reply_final = (mavlink_device_op_read_reply_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), device_op_read_reply_final, MAVLINK_MSG_ID_DEVICE_OP_READ_REPLY_LEN);
...
static inline void mavlink_msg_device_op_read_reply_decode(const mavlink_message_t* msg, mavlink_device_op_read_reply_t* device_op_read_reply)
{
   ...
   // };
    ...
    memcpy(device_op_read_reply, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(device_op_read_reply, decrypt_char, len);
