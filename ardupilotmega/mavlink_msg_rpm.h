                               ...
                               float rpm1, float rpm2)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_RPM_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_RPM_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_rpm_t* rpm_final = (mavlink_rpm_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), rpm_final, MAVLINK_MSG_ID_RPM_LEN);
...
static inline void mavlink_msg_rpm_decode(const mavlink_message_t* msg, mavlink_rpm_t* rpm)
{
   ...
   // };
    ...
    memcpy(rpm, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
