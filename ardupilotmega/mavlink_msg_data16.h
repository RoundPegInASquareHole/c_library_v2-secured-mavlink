                               ...
                               uint8_t type, uint8_t len, const uint8_t *data)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_DATA16_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_DATA16_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_data16_t* data16_final = (mavlink_data16_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), data16_final, MAVLINK_MSG_ID_DATA16_LEN);
...
static inline void mavlink_msg_data16_decode(const mavlink_message_t* msg, mavlink_data16_t* data16)
{
   ...
   // };
    ...
    memcpy(data16, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(data16, decrypt_char, len);
