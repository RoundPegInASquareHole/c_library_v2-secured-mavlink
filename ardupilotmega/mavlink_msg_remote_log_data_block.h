                               ...
                               uint8_t target_system, uint8_t target_component, uint32_t seqno, const uint8_t *data)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_REMOTE_LOG_DATA_BLOCK_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_REMOTE_LOG_DATA_BLOCK_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_remote_log_data_block_t* remote_log_data_block_final = (mavlink_remote_log_data_block_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), remote_log_data_block_final, MAVLINK_MSG_ID_REMOTE_LOG_DATA_BLOCK_LEN);
...
static inline void mavlink_msg_remote_log_data_block_decode(const mavlink_message_t* msg, mavlink_remote_log_data_block_t* remote_log_data_block)
{
   ...
   // };
    ...
    memcpy(remote_log_data_block, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(remote_log_data_block, decrypt_char, len);
