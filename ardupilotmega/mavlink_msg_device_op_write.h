                               ...
                               uint8_t target_system, uint8_t target_component, uint32_t request_id, uint8_t bustype, uint8_t bus, uint8_t address, const char *busname, uint8_t regstart, uint8_t count, const uint8_t *data, uint8_t bank)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_DEVICE_OP_WRITE_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_DEVICE_OP_WRITE_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_device_op_write_t* device_op_write_final = (mavlink_device_op_write_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), device_op_write_final, MAVLINK_MSG_ID_DEVICE_OP_WRITE_LEN);
...
static inline void mavlink_msg_device_op_write_decode(const mavlink_message_t* msg, mavlink_device_op_write_t* device_op_write)
{
   ...
   // };
    ...
    memcpy(device_op_write, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(device_op_write, decrypt_char, len);
