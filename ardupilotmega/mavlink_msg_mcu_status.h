                               ...
                               uint8_t id, int16_t MCU_temperature, uint16_t MCU_voltage, uint16_t MCU_voltage_min, uint16_t MCU_voltage_max)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_MCU_STATUS_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_MCU_STATUS_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_mcu_status_t* mcu_status_final = (mavlink_mcu_status_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), mcu_status_final, MAVLINK_MSG_ID_MCU_STATUS_LEN);
...
static inline void mavlink_msg_mcu_status_decode(const mavlink_message_t* msg, mavlink_mcu_status_t* mcu_status)
{
   ...
   // };
    ...
    memcpy(mcu_status, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(mcu_status, decrypt_char, len);
