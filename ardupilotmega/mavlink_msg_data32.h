                               ...
                               uint8_t type, uint8_t len, const uint8_t *data)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_DATA32_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_DATA32_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_data32_t* data32_final = (mavlink_data32_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), data32_final, MAVLINK_MSG_ID_DATA32_LEN);
...
static inline void mavlink_msg_data32_decode(const mavlink_message_t* msg, mavlink_data32_t* data32)
{
   ...
   // };
    ...
    memcpy(data32, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(data32, decrypt_char, len);
