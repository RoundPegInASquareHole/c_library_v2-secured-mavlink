                               ...
                               uint16_t adc1, uint16_t adc2, uint16_t adc3, uint16_t adc4, uint16_t adc5, uint16_t adc6)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_AP_ADC_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_AP_ADC_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_ap_adc_t* ap_adc_final = (mavlink_ap_adc_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), ap_adc_final, MAVLINK_MSG_ID_AP_ADC_LEN);
...
static inline void mavlink_msg_ap_adc_decode(const mavlink_message_t* msg, mavlink_ap_adc_t* ap_adc)
{
   ...
   // };
    ...
    memcpy(ap_adc, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(ap_adc, decrypt_char, len);
