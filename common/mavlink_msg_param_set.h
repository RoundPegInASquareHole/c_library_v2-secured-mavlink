                               ...
                               uint8_t target_system, uint8_t target_component, const char *param_id, float param_value, uint8_t param_type)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_PARAM_SET_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_PARAM_SET_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_param_set_t* param_set_final = (mavlink_param_set_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), param_set_final, MAVLINK_MSG_ID_PARAM_SET_LEN);
...
static inline void mavlink_msg_param_set_decode(const mavlink_message_t* msg, mavlink_param_set_t* param_set)
{
   ...
   // };
    ...
    memcpy(param_set, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(param_set, decrypt_char, len);
