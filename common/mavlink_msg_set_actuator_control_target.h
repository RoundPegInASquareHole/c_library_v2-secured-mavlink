                               ...
                               uint64_t time_usec, uint8_t group_mlx, uint8_t target_system, uint8_t target_component, const float *controls)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_SET_ACTUATOR_CONTROL_TARGET_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_SET_ACTUATOR_CONTROL_TARGET_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_set_actuator_control_target_t* set_actuator_control_target_final = (mavlink_set_actuator_control_target_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), set_actuator_control_target_final, MAVLINK_MSG_ID_SET_ACTUATOR_CONTROL_TARGET_LEN);
...
static inline void mavlink_msg_set_actuator_control_target_decode(const mavlink_message_t* msg, mavlink_set_actuator_control_target_t* set_actuator_control_target)
{
   ...
   // };
    ...
    memcpy(set_actuator_control_target, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(set_actuator_control_target, decrypt_char, len);
