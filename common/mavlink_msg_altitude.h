                               ...
                               uint64_t time_usec, float altitude_monotonic, float altitude_amsl, float altitude_local, float altitude_relative, float altitude_terrain, float bottom_clearance)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_ALTITUDE_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_ALTITUDE_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_altitude_t* altitude_final = (mavlink_altitude_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), altitude_final, MAVLINK_MSG_ID_ALTITUDE_LEN);
...
static inline void mavlink_msg_altitude_decode(const mavlink_message_t* msg, mavlink_altitude_t* altitude)
{
   ...
   // };
    ...
    memcpy(altitude, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(altitude, decrypt_char, len);
