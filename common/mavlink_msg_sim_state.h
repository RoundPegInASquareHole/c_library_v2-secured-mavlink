                               ...
                               float q1, float q2, float q3, float q4, float roll, float pitch, float yaw, float xacc, float yacc, float zacc, float xgyro, float ygyro, float zgyro, float lat, float lon, float alt, float std_dev_horz, float std_dev_vert, float vn, float ve, float vd)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_SIM_STATE_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_SIM_STATE_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_sim_state_t* sim_state_final = (mavlink_sim_state_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), sim_state_final, MAVLINK_MSG_ID_SIM_STATE_LEN);
...
static inline void mavlink_msg_sim_state_decode(const mavlink_message_t* msg, mavlink_sim_state_t* sim_state)
{
   ...
   // };
    ...
    memcpy(sim_state, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(sim_state, decrypt_char, len);
