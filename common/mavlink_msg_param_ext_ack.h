                               ...
                               const char *param_id, const char *param_value, uint8_t param_type, uint8_t param_result)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_PARAM_EXT_ACK_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_PARAM_EXT_ACK_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_param_ext_ack_t* param_ext_ack_final = (mavlink_param_ext_ack_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), param_ext_ack_final, MAVLINK_MSG_ID_PARAM_EXT_ACK_LEN);
...
static inline void mavlink_msg_param_ext_ack_decode(const mavlink_message_t* msg, mavlink_param_ext_ack_t* param_ext_ack)
{
   ...
   // };
    ...
    memcpy(param_ext_ack, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(param_ext_ack, decrypt_char, len);
