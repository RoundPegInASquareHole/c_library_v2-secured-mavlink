                               ...
                               uint64_t usec, float x, float y, float z, const float *covariance, uint8_t reset_counter)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_VISION_SPEED_ESTIMATE_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_VISION_SPEED_ESTIMATE_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_vision_speed_estimate_t* vision_speed_estimate_final = (mavlink_vision_speed_estimate_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), vision_speed_estimate_final, MAVLINK_MSG_ID_VISION_SPEED_ESTIMATE_LEN);
...
static inline void mavlink_msg_vision_speed_estimate_decode(const mavlink_message_t* msg, mavlink_vision_speed_estimate_t* vision_speed_estimate)
{
   ...
   // };
    ...
    memcpy(vision_speed_estimate, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(vision_speed_estimate, decrypt_char, len);
