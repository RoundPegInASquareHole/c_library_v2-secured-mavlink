                               ...
                               uint32_t time_last_baseline_ms, uint8_t rtk_receiver_id, uint16_t wn, uint32_t tow, uint8_t rtk_health, uint8_t rtk_rate, uint8_t nsats, uint8_t baseline_coords_type, int32_t baseline_a_mm, int32_t baseline_b_mm, int32_t baseline_c_mm, uint32_t accuracy, int32_t iar_num_hypotheses)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_GPS2_RTK_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_GPS2_RTK_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_gps2_rtk_t* gps2_rtk_final = (mavlink_gps2_rtk_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), gps2_rtk_final, MAVLINK_MSG_ID_GPS2_RTK_LEN);
...
static inline void mavlink_msg_gps2_rtk_decode(const mavlink_message_t* msg, mavlink_gps2_rtk_t* gps2_rtk)
{
   ...
   // };
    ...
    memcpy(gps2_rtk, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(gps2_rtk, decrypt_char, len);
