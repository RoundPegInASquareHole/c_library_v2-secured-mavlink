                               ...
                               uint8_t target_system, uint8_t control_request, uint8_t version, const char *passkey)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_CHANGE_OPERATOR_CONTROL_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_CHANGE_OPERATOR_CONTROL_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_change_operator_control_t* change_operator_control_final = (mavlink_change_operator_control_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), change_operator_control_final, MAVLINK_MSG_ID_CHANGE_OPERATOR_CONTROL_LEN);
...
static inline void mavlink_msg_change_operator_control_decode(const mavlink_message_t* msg, mavlink_change_operator_control_t* change_operator_control)
{
   ...
   // };
    ...
    memcpy(change_operator_control, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(change_operator_control, decrypt_char, len);
