                               ...
                               uint8_t target_system, uint8_t target_component, uint16_t sequence)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_LOGGING_ACK_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_LOGGING_ACK_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_logging_ack_t* logging_ack_final = (mavlink_logging_ack_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), logging_ack_final, MAVLINK_MSG_ID_LOGGING_ACK_LEN);
...
static inline void mavlink_msg_logging_ack_decode(const mavlink_message_t* msg, mavlink_logging_ack_t* logging_ack)
{
   ...
   // };
    ...
    memcpy(logging_ack, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(logging_ack, decrypt_char, len);
