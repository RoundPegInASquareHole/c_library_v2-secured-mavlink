                               ...
                               uint8_t compass_id, uint8_t cal_mask, uint8_t cal_status, uint8_t autosaved, float fitness, float ofs_x, float ofs_y, float ofs_z, float diag_x, float diag_y, float diag_z, float offdiag_x, float offdiag_y, float offdiag_z, float orientation_confidence, uint8_t old_orientation, uint8_t new_orientation, float scale_factor)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_MAG_CAL_REPORT_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_MAG_CAL_REPORT_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_mag_cal_report_t* mag_cal_report_final = (mavlink_mag_cal_report_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), mag_cal_report_final, MAVLINK_MSG_ID_MAG_CAL_REPORT_LEN);
...
static inline void mavlink_msg_mag_cal_report_decode(const mavlink_message_t* msg, mavlink_mag_cal_report_t* mag_cal_report)
{
   ...
   // };
    ...
    memcpy(mag_cal_report, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(mag_cal_report, decrypt_char, len);
