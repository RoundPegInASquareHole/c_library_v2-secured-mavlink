                               ...
                               uint32_t MMSI, int32_t lat, int32_t lon, uint16_t COG, uint16_t heading, uint16_t velocity, int8_t turn_rate, uint8_t navigational_status, uint8_t type, uint16_t dimension_bow, uint16_t dimension_stern, uint8_t dimension_port, uint8_t dimension_starboard, const char *callsign, const char *name, uint16_t tslc, uint16_t flags)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_AIS_VESSEL_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_AIS_VESSEL_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_ais_vessel_t* ais_vessel_final = (mavlink_ais_vessel_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), ais_vessel_final, MAVLINK_MSG_ID_AIS_VESSEL_LEN);
...
static inline void mavlink_msg_ais_vessel_decode(const mavlink_message_t* msg, mavlink_ais_vessel_t* ais_vessel)
{
   ...
   // };
    ...
    memcpy(ais_vessel, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(ais_vessel, decrypt_char, len);
