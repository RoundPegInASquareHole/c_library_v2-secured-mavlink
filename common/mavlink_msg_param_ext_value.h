                               ...
                               const char *param_id, const char *param_value, uint8_t param_type, uint16_t param_count, uint16_t param_index)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_PARAM_EXT_VALUE_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_PARAM_EXT_VALUE_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_param_ext_value_t* param_ext_value_final = (mavlink_param_ext_value_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), param_ext_value_final, MAVLINK_MSG_ID_PARAM_EXT_VALUE_LEN);
...
static inline void mavlink_msg_param_ext_value_decode(const mavlink_message_t* msg, mavlink_param_ext_value_t* param_ext_value)
{
   ...
   // };
    ...
    memcpy(param_ext_value, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(param_ext_value, decrypt_char, len);
