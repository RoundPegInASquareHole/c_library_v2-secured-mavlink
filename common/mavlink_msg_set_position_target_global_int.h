                               ...
                               uint32_t time_boot_ms, uint8_t target_system, uint8_t target_component, uint8_t coordinate_frame, uint16_t type_mask, int32_t lat_int, int32_t lon_int, float alt, float vx, float vy, float vz, float afx, float afy, float afz, float yaw, float yaw_rate)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_SET_POSITION_TARGET_GLOBAL_INT_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_SET_POSITION_TARGET_GLOBAL_INT_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_set_position_target_global_int_t* set_position_target_global_int_final = (mavlink_set_position_target_global_int_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), set_position_target_global_int_final, MAVLINK_MSG_ID_SET_POSITION_TARGET_GLOBAL_INT_LEN);
...
static inline void mavlink_msg_set_position_target_global_int_decode(const mavlink_message_t* msg, mavlink_set_position_target_global_int_t* set_position_target_global_int)
{
   ...
   // };
    ...
    memcpy(set_position_target_global_int, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(set_position_target_global_int, decrypt_char, len);
