                               ...
                               uint64_t time_usec, float radius, uint8_t frame, int32_t x, int32_t y, float z)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_ORBIT_EXECUTION_STATUS_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_ORBIT_EXECUTION_STATUS_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_orbit_execution_status_t* orbit_execution_status_final = (mavlink_orbit_execution_status_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), orbit_execution_status_final, MAVLINK_MSG_ID_ORBIT_EXECUTION_STATUS_LEN);
...
static inline void mavlink_msg_orbit_execution_status_decode(const mavlink_message_t* msg, mavlink_orbit_execution_status_t* orbit_execution_status)
{
   ...
   // };
    ...
    memcpy(orbit_execution_status, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(orbit_execution_status, decrypt_char, len);
