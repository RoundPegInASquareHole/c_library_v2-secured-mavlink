                               ...
                               uint32_t time_boot_ms, int16_t xacc, int16_t yacc, int16_t zacc, int16_t xgyro, int16_t ygyro, int16_t zgyro, int16_t xmag, int16_t ymag, int16_t zmag, int16_t temperature)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_SCALED_IMU3_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_SCALED_IMU3_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_scaled_imu3_t* scaled_imu3_final = (mavlink_scaled_imu3_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), scaled_imu3_final, MAVLINK_MSG_ID_SCALED_IMU3_LEN);
...
static inline void mavlink_msg_scaled_imu3_decode(const mavlink_message_t* msg, mavlink_scaled_imu3_t* scaled_imu3)
{
   ...
   // };
    ...
    memcpy(scaled_imu3, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(scaled_imu3, decrypt_char, len);
