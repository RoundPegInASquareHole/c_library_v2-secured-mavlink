                               ...
                               uint32_t onboard_control_sensors_present, uint32_t onboard_control_sensors_enabled, uint32_t onboard_control_sensors_health, uint16_t load, uint16_t voltage_battery, int16_t current_battery, int8_t battery_remaining, uint16_t drop_rate_comm, uint16_t errors_comm, uint16_t errors_count1, uint16_t errors_count2, uint16_t errors_count3, uint16_t errors_count4)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_SYS_STATUS_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_SYS_STATUS_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_sys_status_t* sys_status_final = (mavlink_sys_status_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), sys_status_final, MAVLINK_MSG_ID_SYS_STATUS_LEN);
...
static inline void mavlink_msg_sys_status_decode(const mavlink_message_t* msg, mavlink_sys_status_t* sys_status)
{
   ...
   // };
    ...
    memcpy(sys_status, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(sys_status, decrypt_char, len);
