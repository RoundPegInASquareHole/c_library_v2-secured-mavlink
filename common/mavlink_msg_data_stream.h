                               ...
                               uint8_t stream_id, uint16_t message_rate, uint8_t on_off)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_DATA_STREAM_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_DATA_STREAM_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_data_stream_t* data_stream_final = (mavlink_data_stream_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), data_stream_final, MAVLINK_MSG_ID_DATA_STREAM_LEN);
...
static inline void mavlink_msg_data_stream_decode(const mavlink_message_t* msg, mavlink_data_stream_t* data_stream)
{
   ...
   // };
    ...
    memcpy(data_stream, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(data_stream, decrypt_char, len);
