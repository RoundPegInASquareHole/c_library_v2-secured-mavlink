                               ...
                               uint8_t stream_id, uint8_t count, uint8_t type, uint16_t flags, float framerate, uint16_t resolution_h, uint16_t resolution_v, uint32_t bitrate, uint16_t rotation, uint16_t hfov, const char *name, const char *uri)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_VIDEO_STREAM_INFORMATION_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_VIDEO_STREAM_INFORMATION_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_video_stream_information_t* video_stream_information_final = (mavlink_video_stream_information_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), video_stream_information_final, MAVLINK_MSG_ID_VIDEO_STREAM_INFORMATION_LEN);
...
static inline void mavlink_msg_video_stream_information_decode(const mavlink_message_t* msg, mavlink_video_stream_information_t* video_stream_information)
{
   ...
   // };
    ...
    memcpy(video_stream_information, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(video_stream_information, decrypt_char, len);
