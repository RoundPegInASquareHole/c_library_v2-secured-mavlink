                               ...
                               uint32_t time_boot_ms, uint8_t port, int16_t chan1_scaled, int16_t chan2_scaled, int16_t chan3_scaled, int16_t chan4_scaled, int16_t chan5_scaled, int16_t chan6_scaled, int16_t chan7_scaled, int16_t chan8_scaled, uint8_t rssi)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_RC_CHANNELS_SCALED_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_RC_CHANNELS_SCALED_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_rc_channels_scaled_t* rc_channels_scaled_final = (mavlink_rc_channels_scaled_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), rc_channels_scaled_final, MAVLINK_MSG_ID_RC_CHANNELS_SCALED_LEN);
...
static inline void mavlink_msg_rc_channels_scaled_decode(const mavlink_message_t* msg, mavlink_rc_channels_scaled_t* rc_channels_scaled)
{
   ...
   // };
    ...
    memcpy(rc_channels_scaled, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(rc_channels_scaled, decrypt_char, len);
