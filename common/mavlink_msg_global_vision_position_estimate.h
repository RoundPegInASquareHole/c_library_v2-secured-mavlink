                               ...
                               uint64_t usec, float x, float y, float z, float roll, float pitch, float yaw, const float *covariance, uint8_t reset_counter)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_GLOBAL_VISION_POSITION_ESTIMATE_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_GLOBAL_VISION_POSITION_ESTIMATE_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_global_vision_position_estimate_t* global_vision_position_estimate_final = (mavlink_global_vision_position_estimate_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), global_vision_position_estimate_final, MAVLINK_MSG_ID_GLOBAL_VISION_POSITION_ESTIMATE_LEN);
...
static inline void mavlink_msg_global_vision_position_estimate_decode(const mavlink_message_t* msg, mavlink_global_vision_position_estimate_t* global_vision_position_estimate)
{
   ...
   // };
    ...
    memcpy(global_vision_position_estimate, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(global_vision_position_estimate, decrypt_char, len);
