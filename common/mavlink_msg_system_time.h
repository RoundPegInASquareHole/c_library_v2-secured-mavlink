                               ...
                               uint64_t time_unix_usec, uint32_t time_boot_ms)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_SYSTEM_TIME_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_SYSTEM_TIME_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_system_time_t* system_time_final = (mavlink_system_time_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), system_time_final, MAVLINK_MSG_ID_SYSTEM_TIME_LEN);
...
static inline void mavlink_msg_system_time_decode(const mavlink_message_t* msg, mavlink_system_time_t* system_time)
{
   ...
   // };
    ...
    memcpy(system_time, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(system_time, decrypt_char, len);
