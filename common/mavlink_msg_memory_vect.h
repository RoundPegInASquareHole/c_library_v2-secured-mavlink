                               ...
                               uint16_t address, uint8_t ver, uint8_t type, const int8_t *value)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_MEMORY_VECT_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_MEMORY_VECT_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_memory_vect_t* memory_vect_final = (mavlink_memory_vect_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), memory_vect_final, MAVLINK_MSG_ID_MEMORY_VECT_LEN);
...
static inline void mavlink_msg_memory_vect_decode(const mavlink_message_t* msg, mavlink_memory_vect_t* memory_vect)
{
   ...
   // };
    ...
    memcpy(memory_vect, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(memory_vect, decrypt_char, len);
