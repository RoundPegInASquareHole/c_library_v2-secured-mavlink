                               ...
                               uint64_t status, uint16_t generator_speed, float battery_current, float load_current, float power_generated, float bus_voltage, int16_t rectifier_temperature, float bat_current_setpoint, int16_t generator_temperature, uint32_t runtime, int32_t time_until_maintenance)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_GENERATOR_STATUS_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_GENERATOR_STATUS_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_generator_status_t* generator_status_final = (mavlink_generator_status_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), generator_status_final, MAVLINK_MSG_ID_GENERATOR_STATUS_LEN);
...
static inline void mavlink_msg_generator_status_decode(const mavlink_message_t* msg, mavlink_generator_status_t* generator_status)
{
   ...
   // };
    ...
    memcpy(generator_status, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(generator_status, decrypt_char, len);
