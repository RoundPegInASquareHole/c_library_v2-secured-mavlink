                               ...
                               uint64_t time_usec, uint8_t sensor_id, uint32_t integration_time_us, float integrated_x, float integrated_y, float integrated_xgyro, float integrated_ygyro, float integrated_zgyro, int16_t temperature, uint8_t quality, uint32_t time_delta_distance_us, float distance)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_HIL_OPTICAL_FLOW_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_HIL_OPTICAL_FLOW_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_hil_optical_flow_t* hil_optical_flow_final = (mavlink_hil_optical_flow_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), hil_optical_flow_final, MAVLINK_MSG_ID_HIL_OPTICAL_FLOW_LEN);
...
static inline void mavlink_msg_hil_optical_flow_decode(const mavlink_message_t* msg, mavlink_hil_optical_flow_t* hil_optical_flow)
{
   ...
   // };
    ...
    memcpy(hil_optical_flow, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(hil_optical_flow, decrypt_char, len);
