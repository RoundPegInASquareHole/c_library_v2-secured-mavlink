                               ...
                               uint64_t time_usec, uint16_t chan1_raw, uint16_t chan2_raw, uint16_t chan3_raw, uint16_t chan4_raw, uint16_t chan5_raw, uint16_t chan6_raw, uint16_t chan7_raw, uint16_t chan8_raw, uint16_t chan9_raw, uint16_t chan10_raw, uint16_t chan11_raw, uint16_t chan12_raw, uint8_t rssi)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_HIL_RC_INPUTS_RAW_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_HIL_RC_INPUTS_RAW_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_hil_rc_inputs_raw_t* hil_rc_inputs_raw_final = (mavlink_hil_rc_inputs_raw_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), hil_rc_inputs_raw_final, MAVLINK_MSG_ID_HIL_RC_INPUTS_RAW_LEN);
...
static inline void mavlink_msg_hil_rc_inputs_raw_decode(const mavlink_message_t* msg, mavlink_hil_rc_inputs_raw_t* hil_rc_inputs_raw)
{
   ...
   // };
    ...
    memcpy(hil_rc_inputs_raw, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(hil_rc_inputs_raw, decrypt_char, len);
