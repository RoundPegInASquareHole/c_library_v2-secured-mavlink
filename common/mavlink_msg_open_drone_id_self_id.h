                               ...
                               uint8_t target_system, uint8_t target_component, const uint8_t *id_or_mac, uint8_t description_type, const char *description)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_OPEN_DRONE_ID_SELF_ID_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_OPEN_DRONE_ID_SELF_ID_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_open_drone_id_self_id_t* open_drone_id_self_id_final = (mavlink_open_drone_id_self_id_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), open_drone_id_self_id_final, MAVLINK_MSG_ID_OPEN_DRONE_ID_SELF_ID_LEN);
...
static inline void mavlink_msg_open_drone_id_self_id_decode(const mavlink_message_t* msg, mavlink_open_drone_id_self_id_t* open_drone_id_self_id)
{
   ...
   // };
    ...
    memcpy(open_drone_id_self_id, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(open_drone_id_self_id, decrypt_char, len);
