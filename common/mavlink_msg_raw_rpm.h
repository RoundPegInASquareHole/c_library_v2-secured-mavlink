                               ...
                               uint8_t index, float frequency)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_RAW_RPM_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_RAW_RPM_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_raw_rpm_t* raw_rpm_final = (mavlink_raw_rpm_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), raw_rpm_final, MAVLINK_MSG_ID_RAW_RPM_LEN);
...
static inline void mavlink_msg_raw_rpm_decode(const mavlink_message_t* msg, mavlink_raw_rpm_t* raw_rpm)
{
   ...
   // };
    ...
    memcpy(raw_rpm, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(raw_rpm, decrypt_char, len);
