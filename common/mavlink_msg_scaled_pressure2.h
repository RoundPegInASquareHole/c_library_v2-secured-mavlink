                               ...
                               uint32_t time_boot_ms, float press_abs, float press_diff, int16_t temperature, int16_t temperature_press_diff)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_SCALED_PRESSURE2_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_SCALED_PRESSURE2_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_scaled_pressure2_t* scaled_pressure2_final = (mavlink_scaled_pressure2_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), scaled_pressure2_final, MAVLINK_MSG_ID_SCALED_PRESSURE2_LEN);
...
static inline void mavlink_msg_scaled_pressure2_decode(const mavlink_message_t* msg, mavlink_scaled_pressure2_t* scaled_pressure2)
{
   ...
   // };
    ...
    memcpy(scaled_pressure2, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(scaled_pressure2, decrypt_char, len);
