                               ...
                               uint8_t target_system, uint8_t target_component, uint64_t time_boot_us, const float *q, uint32_t q_estimated_delay_us, float vx, float vy, float vz, uint32_t v_estimated_delay_us, float feed_forward_angular_velocity_z, uint16_t estimator_status, uint8_t landed_state)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_AUTOPILOT_STATE_FOR_GIMBAL_DEVICE_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_AUTOPILOT_STATE_FOR_GIMBAL_DEVICE_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_autopilot_state_for_gimbal_device_t* autopilot_state_for_gimbal_device_final = (mavlink_autopilot_state_for_gimbal_device_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), autopilot_state_for_gimbal_device_final, MAVLINK_MSG_ID_AUTOPILOT_STATE_FOR_GIMBAL_DEVICE_LEN);
...
static inline void mavlink_msg_autopilot_state_for_gimbal_device_decode(const mavlink_message_t* msg, mavlink_autopilot_state_for_gimbal_device_t* autopilot_state_for_gimbal_device)
{
   ...
   // };
    ...
    memcpy(autopilot_state_for_gimbal_device, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(autopilot_state_for_gimbal_device, decrypt_char, len);
