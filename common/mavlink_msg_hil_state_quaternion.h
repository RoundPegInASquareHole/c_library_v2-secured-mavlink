                               ...
                               uint64_t time_usec, const float *attitude_quaternion, float rollspeed, float pitchspeed, float yawspeed, int32_t lat, int32_t lon, int32_t alt, int16_t vx, int16_t vy, int16_t vz, uint16_t ind_airspeed, uint16_t true_airspeed, int16_t xacc, int16_t yacc, int16_t zacc)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_HIL_STATE_QUATERNION_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_HIL_STATE_QUATERNION_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_hil_state_quaternion_t* hil_state_quaternion_final = (mavlink_hil_state_quaternion_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), hil_state_quaternion_final, MAVLINK_MSG_ID_HIL_STATE_QUATERNION_LEN);
...
static inline void mavlink_msg_hil_state_quaternion_decode(const mavlink_message_t* msg, mavlink_hil_state_quaternion_t* hil_state_quaternion)
{
   ...
   // };
    ...
    memcpy(hil_state_quaternion, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(hil_state_quaternion, decrypt_char, len);
