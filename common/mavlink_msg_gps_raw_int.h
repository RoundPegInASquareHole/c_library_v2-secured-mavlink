                               ...
                               uint64_t time_usec, uint8_t fix_type, int32_t lat, int32_t lon, int32_t alt, uint16_t eph, uint16_t epv, uint16_t vel, uint16_t cog, uint8_t satellites_visible, int32_t alt_ellipsoid, uint32_t h_acc, uint32_t v_acc, uint32_t vel_acc, uint32_t hdg_acc, uint16_t yaw)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_GPS_RAW_INT_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_GPS_RAW_INT_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_gps_raw_int_t* gps_raw_int_final = (mavlink_gps_raw_int_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), gps_raw_int_final, MAVLINK_MSG_ID_GPS_RAW_INT_LEN);
...
static inline void mavlink_msg_gps_raw_int_decode(const mavlink_message_t* msg, mavlink_gps_raw_int_t* gps_raw_int)
{
   ...
   // };
    ...
    memcpy(gps_raw_int, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(gps_raw_int, decrypt_char, len);
