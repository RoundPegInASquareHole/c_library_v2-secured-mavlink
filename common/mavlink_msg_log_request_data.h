                               ...
                               uint8_t target_system, uint8_t target_component, uint16_t id, uint32_t ofs, uint32_t count)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_LOG_REQUEST_DATA_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_LOG_REQUEST_DATA_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_log_request_data_t* log_request_data_final = (mavlink_log_request_data_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), log_request_data_final, MAVLINK_MSG_ID_LOG_REQUEST_DATA_LEN);
...
static inline void mavlink_msg_log_request_data_decode(const mavlink_message_t* msg, mavlink_log_request_data_t* log_request_data)
{
   ...
   // };
    ...
    memcpy(log_request_data, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(log_request_data, decrypt_char, len);
