                               ...
                               uint64_t time_usec, int16_t press_abs, int16_t press_diff1, int16_t press_diff2, int16_t temperature)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_RAW_PRESSURE_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_RAW_PRESSURE_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_raw_pressure_t* raw_pressure_final = (mavlink_raw_pressure_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), raw_pressure_final, MAVLINK_MSG_ID_RAW_PRESSURE_LEN);
...
static inline void mavlink_msg_raw_pressure_decode(const mavlink_message_t* msg, mavlink_raw_pressure_t* raw_pressure)
{
   ...
   // };
    ...
    memcpy(raw_pressure, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(raw_pressure, decrypt_char, len);
