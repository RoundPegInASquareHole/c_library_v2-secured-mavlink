                               ...
                               uint64_t time_usec, const float *q, float rollspeed, float pitchspeed, float yawspeed, const float *covariance)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_ATTITUDE_QUATERNION_COV_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_ATTITUDE_QUATERNION_COV_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_attitude_quaternion_cov_t* attitude_quaternion_cov_final = (mavlink_attitude_quaternion_cov_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), attitude_quaternion_cov_final, MAVLINK_MSG_ID_ATTITUDE_QUATERNION_COV_LEN);
...
static inline void mavlink_msg_attitude_quaternion_cov_decode(const mavlink_message_t* msg, mavlink_attitude_quaternion_cov_t* attitude_quaternion_cov)
{
   ...
   // };
    ...
    memcpy(attitude_quaternion_cov, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(attitude_quaternion_cov, decrypt_char, len);
