                               ...
                               uint32_t time_boot_ms, float roll, float pitch, float yaw, float rollspeed, float pitchspeed, float yawspeed)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_ATTITUDE_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_ATTITUDE_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_attitude_t* attitude_final = (mavlink_attitude_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), attitude_final, MAVLINK_MSG_ID_ATTITUDE_LEN);
...
static inline void mavlink_msg_attitude_decode(const mavlink_message_t* msg, mavlink_attitude_t* attitude)
{
   ...
   // };
    ...
    memcpy(attitude, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(attitude, decrypt_char, len);
