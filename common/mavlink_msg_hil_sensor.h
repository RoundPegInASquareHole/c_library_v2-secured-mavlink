                               ...
                               uint64_t time_usec, float xacc, float yacc, float zacc, float xgyro, float ygyro, float zgyro, float xmag, float ymag, float zmag, float abs_pressure, float diff_pressure, float pressure_alt, float temperature, uint32_t fields_updated, uint8_t id)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_HIL_SENSOR_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_HIL_SENSOR_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_hil_sensor_t* hil_sensor_final = (mavlink_hil_sensor_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), hil_sensor_final, MAVLINK_MSG_ID_HIL_SENSOR_LEN);
...
static inline void mavlink_msg_hil_sensor_decode(const mavlink_message_t* msg, mavlink_hil_sensor_t* hil_sensor)
{
   ...
   // };
    ...
    memcpy(hil_sensor, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(hil_sensor, decrypt_char, len);
