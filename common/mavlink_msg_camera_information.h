                               ...
                               uint32_t time_boot_ms, const uint8_t *vendor_name, const uint8_t *model_name, uint32_t firmware_version, float focal_length, float sensor_size_h, float sensor_size_v, uint16_t resolution_h, uint16_t resolution_v, uint8_t lens_id, uint32_t flags, uint16_t cam_definition_version, const char *cam_definition_uri)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_CAMERA_INFORMATION_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_CAMERA_INFORMATION_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_camera_information_t* camera_information_final = (mavlink_camera_information_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), camera_information_final, MAVLINK_MSG_ID_CAMERA_INFORMATION_LEN);
...
static inline void mavlink_msg_camera_information_decode(const mavlink_message_t* msg, mavlink_camera_information_t* camera_information)
{
   ...
   // };
    ...
    memcpy(camera_information, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(camera_information, decrypt_char, len);
