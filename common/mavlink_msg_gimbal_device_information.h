                               ...
                               uint32_t time_boot_ms, const char *vendor_name, const char *model_name, const char *custom_name, uint32_t firmware_version, uint32_t hardware_version, uint64_t uid, uint16_t cap_flags, uint16_t custom_cap_flags, float roll_min, float roll_max, float pitch_min, float pitch_max, float yaw_min, float yaw_max)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_GIMBAL_DEVICE_INFORMATION_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_GIMBAL_DEVICE_INFORMATION_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_gimbal_device_information_t* gimbal_device_information_final = (mavlink_gimbal_device_information_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), gimbal_device_information_final, MAVLINK_MSG_ID_GIMBAL_DEVICE_INFORMATION_LEN);
...
static inline void mavlink_msg_gimbal_device_information_decode(const mavlink_message_t* msg, mavlink_gimbal_device_information_t* gimbal_device_information)
{
   ...
   // };
    ...
    memcpy(gimbal_device_information, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(gimbal_device_information, decrypt_char, len);
