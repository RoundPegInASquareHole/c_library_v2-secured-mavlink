                               ...
                               int32_t lat, int32_t lon)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_TERRAIN_CHECK_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_TERRAIN_CHECK_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_terrain_check_t* terrain_check_final = (mavlink_terrain_check_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), terrain_check_final, MAVLINK_MSG_ID_TERRAIN_CHECK_LEN);
...
static inline void mavlink_msg_terrain_check_decode(const mavlink_message_t* msg, mavlink_terrain_check_t* terrain_check)
{
   ...
   // };
    ...
    memcpy(terrain_check, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(terrain_check, decrypt_char, len);
