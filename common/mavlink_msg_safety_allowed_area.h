                               ...
                               uint8_t frame, float p1x, float p1y, float p1z, float p2x, float p2y, float p2z)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_SAFETY_ALLOWED_AREA_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_SAFETY_ALLOWED_AREA_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_safety_allowed_area_t* safety_allowed_area_final = (mavlink_safety_allowed_area_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), safety_allowed_area_final, MAVLINK_MSG_ID_SAFETY_ALLOWED_AREA_LEN);
...
static inline void mavlink_msg_safety_allowed_area_decode(const mavlink_message_t* msg, mavlink_safety_allowed_area_t* safety_allowed_area)
{
   ...
   // };
    ...
    memcpy(safety_allowed_area, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(safety_allowed_area, decrypt_char, len);
