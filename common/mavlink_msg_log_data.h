                               ...
                               uint16_t id, uint32_t ofs, uint8_t count, const uint8_t *data)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_LOG_DATA_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_LOG_DATA_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_log_data_t* log_data_final = (mavlink_log_data_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), log_data_final, MAVLINK_MSG_ID_LOG_DATA_LEN);
...
static inline void mavlink_msg_log_data_decode(const mavlink_message_t* msg, mavlink_log_data_t* log_data)
{
   ...
   // };
    ...
    memcpy(log_data, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(log_data, decrypt_char, len);
