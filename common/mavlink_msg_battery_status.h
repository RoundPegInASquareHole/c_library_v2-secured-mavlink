                               ...
                               uint8_t id, uint8_t battery_function, uint8_t type, int16_t temperature, const uint16_t *voltages, int16_t current_battery, int32_t current_consumed, int32_t energy_consumed, int8_t battery_remaining, int32_t time_remaining, uint8_t charge_state, const uint16_t *voltages_ext, uint8_t mode, uint32_t fault_bitmask)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_BATTERY_STATUS_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_BATTERY_STATUS_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_battery_status_t* battery_status_final = (mavlink_battery_status_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), battery_status_final, MAVLINK_MSG_ID_BATTERY_STATUS_LEN);
...
static inline void mavlink_msg_battery_status_decode(const mavlink_message_t* msg, mavlink_battery_status_t* battery_status)
{
   ...
   // };
    ...
    memcpy(battery_status, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(battery_status, decrypt_char, len);
