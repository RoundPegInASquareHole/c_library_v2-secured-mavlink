                               ...
                               float airspeed, float groundspeed, int16_t heading, uint16_t throttle, float alt, float climb)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_VFR_HUD_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_VFR_HUD_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_vfr_hud_t* vfr_hud_final = (mavlink_vfr_hud_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), vfr_hud_final, MAVLINK_MSG_ID_VFR_HUD_LEN);
...
static inline void mavlink_msg_vfr_hud_decode(const mavlink_message_t* msg, mavlink_vfr_hud_t* vfr_hud)
{
   ...
   // };
    ...
    memcpy(vfr_hud, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(vfr_hud, decrypt_char, len);
