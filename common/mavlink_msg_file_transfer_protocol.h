                               ...
                               uint8_t target_network, uint8_t target_system, uint8_t target_component, const uint8_t *payload)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_FILE_TRANSFER_PROTOCOL_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_FILE_TRANSFER_PROTOCOL_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_file_transfer_protocol_t* file_transfer_protocol_final = (mavlink_file_transfer_protocol_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), file_transfer_protocol_final, MAVLINK_MSG_ID_FILE_TRANSFER_PROTOCOL_LEN);
...
static inline void mavlink_msg_file_transfer_protocol_decode(const mavlink_message_t* msg, mavlink_file_transfer_protocol_t* file_transfer_protocol)
{
   ...
   // };
    ...
    memcpy(file_transfer_protocol, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(file_transfer_protocol, decrypt_char, len);
