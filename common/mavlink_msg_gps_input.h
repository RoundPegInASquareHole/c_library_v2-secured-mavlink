                               ...
                               uint64_t time_usec, uint8_t gps_id, uint16_t ignore_flags, uint32_t time_week_ms, uint16_t time_week, uint8_t fix_type, int32_t lat, int32_t lon, float alt, float hdop, float vdop, float vn, float ve, float vd, float speed_accuracy, float horiz_accuracy, float vert_accuracy, uint8_t satellites_visible, uint16_t yaw)
{
    ...
    uint8_t encrypt[MAVLINK_MSG_ID_GPS_INPUT_LEN];
    ChaCha20XOR(chacha20_key, 1, nonce, (uint8_t *)packet_char, (uint8_t *)encrypt, MAVLINK_MSG_ID_GPS_INPUT_LEN);
    const char* encrypt_char = (const char*) &encrypt;
    
    mavlink_gps_input_t* gps_input_final = (mavlink_gps_input_t*)encrypt_char;
    memcpy(_MAV_PAYLOAD_NON_CONST(msg), gps_input_final, MAVLINK_MSG_ID_GPS_INPUT_LEN);
...
static inline void mavlink_msg_gps_input_decode(const mavlink_message_t* msg, mavlink_gps_input_t* gps_input)
{
   ...
   // };
    ...
    memcpy(gps_input, _MAV_PAYLOAD(msg), len); // this is the original way to decode the incomming payload
    ...
    //memcpy(gps_input, decrypt_char, len);
